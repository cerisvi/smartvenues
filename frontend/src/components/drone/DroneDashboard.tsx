...
import type { SimulationParams, DashboardTab, DroneHub } from '../../types/drone';
...
const TABS: { id: DashboardTab; label: string; icon: string }[] = [
  ...
  { id: 'planner',    label: 'Pannello Volo',     icon: '✈️' },
  { id: 'history',    label: 'Storico & Confronto', icon: '📂' },
];
...
export default function DroneDashboard() {
  ...
  const [plannerHubs, setPlannerHubs] = useState<DroneHub[] | null>(null);
  ...
  // Called when user presses "Vedi su Mappa" in planner or history
  const handleShowOnMap = useCallback((hubs: DroneHub[]) => {
    setPlannerHubs(hubs);
    setActiveTab('map');
    setAnimating(false);
  }, []);
  ...
  return (
    <div className="flex flex-col h-full bg-slate-950 text-white overflow-hidden">
      ...
      <header className="flex items-center gap-0 bg-slate-900 border-b border-slate-700 shrink-0 z-20">
        ...
        {/* Tabs */}
        <nav className="flex items-center flex-1 px-2 overflow-x-auto">
          {TABS.map((tab) => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
              className={`
                flex items-center gap-1.5 px-4 py-3 text-xs font-medium transition-all border-b-2 whitespace-nowrap
                ${activeTab === tab.id
        ...
        {/* Sidebar toggle (only for map/simulation) */}
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* Simulation params sidebar */}
        {(activeTab === 'map' || activeTab === 'simulation') && sidebarOpen && (
          <div className="w-72 shrink-0 overflow-hidden">
            <SimulationPanel params={params} onChange={handleParamChange} onReset={handleReset} />
          </div>
        )}
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          {activeTab === 'map' && (
            <div className="flex-1 overflow-hidden">
              <DroneMap
                ...
                plannerHubs={plannerHubs}
                onClearPlannerRoute={() => setPlannerHubs(null)}
              />
            </div>
          )}
          ...
          {activeTab === 'planner' && (
            <div className="flex-1 overflow-hidden">
              <DroneRoutePlanner onShowOnMap={handleShowOnMap} />
            </div>
          )}

          {activeTab === 'history' && (
            <div className="flex-1 overflow-hidden">
              <FlightHistory onShowOnMap={handleShowOnMap} />
            </div>
          )}

          {/* KPI bar */}
          {activeTab !== 'report' && activeTab !== 'planner' && activeTab !== 'history' && (
...
function SimulationOverview({ result }: OverviewProps) {
  ...
  const blocks = [
    {
      title: 'Rotta', icon: '📍', color: 'border-cyan-500/30 bg-cyan-500/5',
      items: [
    ...
    {
      title: 'Costi Logistici', icon: '💰', color: 'border-green-500/30 bg-green-500/5',
      items: [
    ...
    {
      title: 'Impatto Ambientale', icon: '🌍', color: 'border-emerald-500/30 bg-emerald-500/5',
      items: [
        { label: 'CO₂ drone/consegna',      value: `${(environment.droneCo2PerDeliveryKg * 1000).toFixed(0)}g`, highlight: true },
        { label: `CO₂ ${vehicle.label}/cons.`, value: `${(environment.vehicleCo2PerDeliveryKg * 1000).toFixed(0)}g` },
    ...
    {
      title: 'Riduzione Traffico', icon: '🚗', color: 'border-amber-500/30 bg-amber-500/5',
      items: [
    ...
    {
      title: 'Analisi Finanziaria', icon: '📈', color: 'border-purple-500/30 bg-purple-500/5',
      items: [
    ...
    {
      title: 'Operativo & Sociale', icon: '⚙️', color: 'border-indigo-500/30 bg-indigo-500/5',
      items: [
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="mb-6 flex items-center gap-3">
        <div className="text-lg font-bold text-white">Risultati Simulazione — {drone.name}</div>
