...
function computeRoute(form: PlannerForm): PlannerResult {
  ...
  // Build ordered hub list
  const hubIds = [
    form.originHubId,
    ...form.waypoints.map((w) => w.hubId),
  ...
  if (form.payloadKg > deliveryType.maxKg) {
    warnings.push(`Il peso (${form.payloadKg} kg) supera il limite per il tipo "${deliveryType.label}" (max ${deliveryType.maxKg} kg).`);
  }

  // Pick best drone automatically or use selected
  ...
  if (drone && drone.maxPayloadKg < form.payloadKg) {
  ...
  for (let i = 0; i < hubs.length - 1; i++) {
    ...
    const energyCost = parseFloat((distKm * drone.energyKwhPerKm * 0.22).toFixed(2)); // €0.22/kWh
  ...
  const operatorCost = parseFloat(((totalFlightTimeMin / 60) * 22).toFixed(2)); // €22/h operator
  ...
  const droneCo2Kg = droneEnergyKwh * 0.233; // Italian grid avg
  const vanCo2Kg = (totalDistanceKm * 1.3 * 185) / 1000; // road distance ~1.3x, 185g/km
  const co2SavedKg = parseFloat((vanCo2Kg - droneCo2Kg).toFixed(3));

  // Success probability
  const successProbability = Math.round(
    weather.successRate * (form.urgencyLevel === 'critical' ? 0.97 : 1.0) * 100
  );
  ...
  return {
    legs,
    recommendedDroneId: drone?.id ?? 'dji-flycart30',
    totalDistanceKm,
    totalFlightTimeMin,
    totalCostEur,
...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return (
    <h3 className="text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-2">{children}</h3>
  );
}
...
export default function DroneRoutePlanner() {
  ...
  const addWaypoint = useCallback(() => {
    ...
    setField('waypoints', [
      ...form.waypoints,
      { id: crypto.randomUUID(), hubId: next.id, note: '' },
    ]);
  }, [form, setField]);
  ...
  const updateWaypoint = useCallback((id: string, hubId: string) => {
    setForm((f) => ({
      ...f,
      waypoints: f.waypoints.map((w) => (w.id === id ? { ...w, hubId } : w)),
    }));
    setSubmitted(false);
  }, []);
    ...
    <div className="flex h-full overflow-hidden">
      ...
      <div className="w-80 shrink-0 bg-slate-900 border-r border-slate-700 overflow-y-auto flex flex-col">
        <div className="px-4 py-3 border-b border-slate-700 shrink-0">
          <div className="text-sm font-bold text-white">Calcola Preventivo Rotta</div>
          <div className="text-[10px] text-slate-500 mt-0.5">Inserisci i parametri della consegna</div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-5">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  onChange={(e) => setField('originHubId', e.target.value)}
                  className={selectCls}
                >
                  {CATANIA_HUBS.map((h) => (
                    <option key={h.id} value={h.id}>{h.shortName} — {h.type}</option>
                  ))}
                </select>
              </div>
              ...
              {form.waypoints.map((wp, idx) => (
                <div key={wp.id} className="flex items-center gap-2">
                  ...
                  >
                    <Trash2 size={11} className="text-slate-500 hover:text-rose-400" />
                  </button>
                </div>
              ))}
              ...
              {form.waypoints.length < 5 && (
                <button
                  onClick={addWaypoint}
                  className="flex items-center gap-1.5 text-xs text-cyan-500 hover:text-cyan-300 transition-colors ml-6"
                >
                  <Plus size={12} />
              ...
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  onChange={(e) => setField('destinationHubId', e.target.value)}
                  className={selectCls}
                >
                  {CATANIA_HUBS.map((h) => (
                    <option key={h.id} value={h.id}>{h.shortName} — {h.type}</option>
          ...
          <div>
            ...
            <Field label="Modello drone">
              <select
                value={form.droneModelId}
                onChange={(e) => setField('droneModelId', e.target.value)}
                className={selectCls}
              >
                <option value="auto">🤖 Automatico (consigliato)</option>
                {DRONE_MODELS.map((d) => (
                  <option key={d.id} value={d.id}>
                    {d.name} — max {d.maxPayloadKg}kg / {d.maxRangeKm}km
                  </option>
                ))}
              </select>
            </Field>
          ...
          <div>
            ...
            <div className="space-y-2">
              <Field label="Meteo">
                <select value={form.weatherCondition} onChange={(e) => setField('weatherCondition', e.target.value as any)} className={selectCls}>
              ...
              <Field label="Urgenza">
                <select value={form.urgencyLevel} onChange={(e) => setField('urgencyLevel', e.target.value as any)} className={selectCls}>
        ...
        <div className="px-4 py-3 border-t border-slate-700 shrink-0">
          <button
            ...
            className="w-full py-2.5 bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-bold text-sm rounded-xl transition-colors"
          >
            🚁 Calcola Preventivo
          </button>
          <button
            onClick={() => { setForm(DEFAULT_FORM); setSubmitted(false); }}
            className="w-full mt-1.5 py-1.5 text-xs text-slate-500 hover:text-slate-300 transition-colors"
          >
            Reimposta
          </button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto bg-slate-950 p-6">
        {!submitted && (
          <div className="h-full flex flex-col items-center justify-center text-center gap-4 opacity-50">
            ...
            <div className="text-slate-400 text-sm">Configura la rotta nel pannello a sinistra<br />e premi <b className="text-slate-200">Calcola Preventivo</b></div>
          </div>
        )}
        ...
        {submitted && result && (
          <div className="max-w-3xl mx-auto space-y-5">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <h2 className="text-base font-bold text-white">Preventivo Rotta</h2>
              ...
              <span className="text-xs px-2.5 py-1 rounded-full border bg-slate-800 border-slate-700 text-slate-300">
                {DELIVERY_TYPES.find((d) => d.id === form.deliveryType)?.icon} {DELIVERY_TYPES.find((d) => d.id === form.deliveryType)?.label}
              </span>
              <span className="text-xs text-slate-500 ml-auto">{form.preferredTimeWindow}</span>
            </div>
                ...
                {result.legs.map((leg, i) => (
                  <div key={i} className="bg-slate-900 rounded-xl border border-slate-700 p-3">
                    ...
                    <div className="grid grid-cols-4 gap-2 text-[10px]">
                      ...
                      <div>
                        <span className="text-slate-500">Ricariche</span><br />
                        <b className={leg.batteryStopsNeeded > 0 ? 'text-amber-300' : 'text-slate-400'}>
                          {leg.batteryStopsNeeded > 0 ? `${leg.batteryStopsNeeded} sosta` : '—'}
                        </b>
                      </div>
                    </div>
                  </div>
            ...
            <div className="bg-slate-900 rounded-xl border border-slate-700 p-4">
              ...
              <div className="space-y-2">
                {[
                  ...
                  const pct = result.totalCostEur > 0 ? Math.round((value / (result.costBreakdown.energy + result.costBreakdown.maintenance + result.costBreakdown.operator + result.costBreakdown.permits)) * 100) : 0;
                  return (
